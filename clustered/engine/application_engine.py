...
class ApplicationEngine(ApplicationBase):
    ...
    def _setup_meta_db(self, env_config_file, env_config) -> None:
        try:
    ...
    def initiate(self, **kwargs) -> None:
        ...
        env_config = _get_config("ENVIRON_CONFIG", kwargs.get('env_config_file', ''), 'config/Environment_config.json', True)
        self._build_configuration(wrkspc, "ENVIRON_CONFIG", env_config)
        repo_config = _get_config("REPOSITORY_CONFIG", kwargs.get('repo_config_file', ''), 'config/Repository_config.json', True)
        self._build_configuration(wrkspc, "REPOSITORY_CONFIG", repo_config)
        clus_config = _get_config("CLUSTER_CONFIG", kwargs.get('clus_config_file', ''), 'config/Cluster_config.json', True)
        self._build_configuration(wrkspc, "CLUSTER_CONFIG", clus_config)
        pnode_config = _get_config("PARENT_NODE_CONFIG", kwargs.get('pnode_config_file', ''), 'config/Parenet_Node_config.json', True)
        self._build_configuration(wrkspc, "PARENT_NODE_CONFIG", pnode_config)
        cnode_config = _get_config("CHILD_NODE_CONFIG", kwargs.get('cnode_config_file', ''), 'config/Child_Node_config.json', True)
        self._build_configuration(wrkspc, "CHILD_NODE_CONFIG", cnode_config)
        self._setup_meta_db(env_config_file, env_config)

    def refresh(self, **kwargs) -> None:
        wrkspc = Engine().isinitiated()
        repo_config = _get_config("REPOSITORY_CONFIG", kwargs.get('repo_config_file', ''), 'config/Repository_config.json')
        self._build_configuration(wrkspc, "REPOSITORY_CONFIG", repo_config)
        clus_config = _get_config("CLUSTER_CONFIG", kwargs.get('clus_config_file', ''), 'config/Cluster_config.json')
        self._build_configuration(wrkspc, "CLUSTER_CONFIG", clus_config)
        pnode_config = _get_config("PARENT_NODE_CONFIG", kwargs.get('pnode_config_file', ''), 'config/Parenet_Node_config.json')
        self._build_configuration(wrkspc, "PARENT_NODE_CONFIG", pnode_config)
        cnode_config = _get_config("CHILD_NODE_CONFIG", kwargs.get('cnode_config_file', ''), 'config/Child_Node_config.json')
        self._build_configuration(wrkspc, "CHILD_NODE_CONFIG", cnode_config)
    ...
    def destroy(self) -> None:
        try:
            self.isinitiated()
            wrkspc = self._get_wrkspc()
            env_config = pickle.load((wrkspc / ("." + "ENVIRON_CONFIG.pkl")).as_posix())
